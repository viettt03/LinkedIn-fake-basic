...
const index = () => {
    ...
    const fetchFriendRequests = async () => {
        try {
            const response = await axios.get(`http://10.0.2.2:3000/connection-request/${userId}`);
            if (response.status === 200) {

                const connectionRequestsData = response.data?.map((friendRequest) => ({
                    _id: friendRequest._id,
                    name: friendRequest.name,
                    email: friendRequest.name,
    ...
    return (
        <ScrollView
            ...
            <Pressable
                ...
                style={{
                    marginTop: 10, marginHorizontal: 10, flexDirection: 'row', alignItems: 'center',
            ...
            <View style={{ marginHorizontal: 15 }}>
                ...
                <View style={{
                    backgroundColor: '#FFC72C', width: 140, paddingVertical: 5, borderRadius: 25,
                    marginTop: 8
                }}>
